...
envSample<- function (coord, filters, res, do.plot=TRUE){
  # 
  # coord <- coord
  # filters <- list(data$wc2.1_2.5m_bio_1, data$wc2.1_2.5m_bio_1)
  # res <- list( diff(range(filters[[1]]))/40, 
  #              diff(range(filters[[2]]))/40)
  # 
  n<- length (filters)
  pot_points<- list ()
  for (i in 1:n){
  ...
  rows <- !duplicated(selection$groupID)
  ...
  if (do.plot==TRUE){
    ...
    map(add=T)
    points (coord_filter, pch='+', col="forestgreen")
    par(mfrow=c(1,1))
  }
  # coord_filter
  rows
